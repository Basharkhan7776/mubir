  ...
  reducers: {
    ...
    addTransaction: (
      ...
      const entry = state.entries.find(
        (e) => e.organization.id === action.payload.organizationId,
      );
      if (entry) {
    ...
    updateTransaction: (
      ...
      if (entry) {
        const transaction = entry.transactions.find(
    ...
    deleteOrganization: (state, action: PayloadAction<string>) => {
      state.entries = state.entries.filter(
        (e) => e.organization.id !== action.payload,
      );
    },
    deleteTransactions: (
      state,
      ...
      const entryIndex = state.entries.findIndex(
        (e) => e.organization.id === organizationId,
      );
      if (entryIndex !== -1) {
        state.entries[entryIndex].transactions = state.entries[
          entryIndex
